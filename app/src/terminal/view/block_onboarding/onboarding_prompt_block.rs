...
impl OnboardingPromptBlock {
    ...
    fn render_text(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINE_ONE: &str = "Next, let’s set up your prompt. Tilde has a custom prompt builder or you can select PS1 to honor your pre-existing prompt configuration.";
        const LINE_TWO: &str =
            "Tilde works with many custom prompts like oh-my-zsh, Starship, and Powerlevel10K. ";
        const LINK_TEXT: &str = "Report an issue";
        const LINK_DESTINATION: &str = "https://github.com/nguyenphutrong/tilde/issues";
    ...
    fn render_existing_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        const LINK_DESTINATION: &str = "https://github.com/nguyenphutrong/tilde/issues/new";
    ...
    fn render_warp_prompt_button_interior(&self, appearance: &Appearance) -> Box<dyn Element> {
        // Pixel values pulled from Figma mocks
        // https://www.figma.com/file/y888viqzWBoMpFTxQqkQEN/Activation?node-id=568:1595&mode=dev
        const HEADER_TEXT: &str = "Tilde prompt";
        const HEADER_MARGIN_LEFT: f32 = 4.;
