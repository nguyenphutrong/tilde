            ...
            move |view, result, ctx| {
                ...
                match result {
                    Ok(()) => {
                        ...
                        let message = format!(
                            "Successfully installed the Tilde Control CLI! You can now run '{command_name}' from the command line."
                        );
                    ...
                    Err(error) => {
                        let message = format!("Failed to install Tilde Control command: {error}");
                        log::warn!("{message}");
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
...
impl SettingsWidget for WarpControlCliInstallWidget {
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            "Tilde Control CLI command".into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            button,
            Some("Install the warpctrl command for scripting Tilde from your terminal.".to_owned()),
        )
    }
}
...
impl SettingsWidget for LocalControlModeWidget {
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            ...
            Some("warpctrl allows for scripting Tilde's UI. Use with care.".to_owned()),
        )
    }
}
