...
use crate::app_state::{
    ...
    SettingsPaneSnapshot, TabGroupSnapshot, TabSnapshot, TerminalPaneSnapshot, WindowSnapshot,
...
// Include the product name so it remains recognizable when users screen-share an available update.
const UPDATE_READY_TEXT: &str = "Update Tilde";
...
const VERSION_DEPRECATION_WITHOUT_PERMISSIONS_BANNER_TEXT: &str = "Some Tilde features may not work as expected without updating immediately, but Tilde is unable to perform the update.";
...
pub struct Workspace {
    ...
    agent_management_filters: Option<PersistedAgentManagementFilters>,
...
impl Workspace {
    ...
    /// The current focusable panels are local tools, theme chooser, and resource center.
    fn focus_left_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Starts from the resource center right panel.
        else if self.resource_center_view.is_self_or_child_focused(ctx) {
            self.focus_active_tab(ctx);
        }
        // Starts from the local tools panel.
    ...
    fn focus_right_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.active_tab_pane_group().is_self_or_child_focused(ctx) {
            if self.current_workspace_state.is_resource_center_open {
                ctx.focus(&self.resource_center_view);
            } else if self.active_tab_pane_group().as_ref(ctx).left_panel_open {
                ctx.focus(&self.left_panel_view);
            } else if self.is_theme_chooser_open() {
                ctx.focus(&self.theme_chooser_view);
            }
        }
        // Starts from a left panel: local tools or theme chooser.
        else if self.left_panel_view.is_self_or_child_focused(ctx)
            ...
            && let Some(version) = ChannelState::app_version()
        {
            ...
            match autoupdate::get_update_state(ctx) {
                ...
                AutoupdateStage::UnableToUpdateToNewVersion { .. } => menu_items.push(
                    MenuItemFields::new("Update Tilde manually")
    ...
    fn install_oz(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(async { cli_install::install_oz() }, |view, result, ctx| {
            ...
            let message = format!("Installed the Oz CLI globally. You can now run '{command_name}' from any terminal outside of Tilde.");
        ...
        ctx.spawn(
            ...
            |view, result, ctx| {
                let toast = DismissibleToast::success(
                    "Removed the global Oz CLI installation — it still works inside Tilde."
                        .to_string(),
                );
    ...
    fn install_warpctrl(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            ...
            |view, result, ctx| {
                ...
                let message = format!("Installed the Tilde Control CLI globally. You can now run '{command_name}' from any terminal outside of Tilde.");
                let toast = DismissibleToast::success(message);
                view.handle_cli_command_result(
                    result,
                    toast,
                    "Failed to install Tilde Control command",
                    ctx,
                );
            },
        );
    ...
    fn uninstall_warpctrl(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            ...
            |view, result, ctx| {
                let toast = DismissibleToast::success(
                    "Removed the global Tilde Control CLI installation — it still works inside Tilde."
                        .to_string(),
                );
                view.handle_cli_command_result(
                    result,
                    toast,
                    "Failed to uninstall Tilde Control command",
                    ctx,
                );
            },
        );
    ...
    fn user_menu_items(&self, app: &AppContext) -> Vec<MenuItem<WorkspaceAction>> {
            ...
            && ChannelState::show_autoupdate_menu_items()
        {
            match autoupdate::get_update_state(app) {
                ...
                {
                    items.push(
                        MenuItemFields::new("Update and relaunch Tilde")
                            .with_on_select_action(WorkspaceAction::ApplyUpdate)
        ...
        #[cfg(not(target_family = "wasm"))]
        items.push(
            MenuItemFields::new("View Tilde logs")
    ...
    pub fn snapshot(
        ...
        WindowSnapshot {
            ...
            left_panel_width,
            right_panel_width,
            agent_management_filters: self.agent_management_filters.clone(),
            tab_groups,
        }
    }
                ...
                match &outcome {
                    ...
                    RequestPermissionsOutcome::PermissionsDenied => {
                        ...
                        view.toast_stack.update(ctx, |toast_stack, ctx| {
                            let toast = DismissibleToast::error(
                                "Tilde doesn't have permission to send desktop notifications."
                                    .to_string(),
                            )
                            .with_link(
                ...
                if let Some(version) = ChannelState::app_version() {
                    ...
                    if FeatureFlag::AvatarInTabBar.is_enabled() {
                        self.update_toast_stack.update(ctx, |stack, ctx| {
                            ...
                            let toast = DismissibleToast::default(String::from("Tilde updated!"))
                                .with_link(link);
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::ViewInWarpDrive(_) => {}
            ...
            pane_group::Event::ToggleLeftPanel {
                ...
                let is_target_active = self.left_panel_view.as_ref(ctx).is_file_tree_active();
            ...
            pane_group::Event::ToggleLeftPanel {
                target_view: LeftPanelTargetView::WarpDrive,
                ..
            } => {}
            #[cfg(feature = "local_fs")]
        ...
        let secondary_button =
            AISettings::as_ref(app)
                ...
                .then(|| WorkspaceBannerButtonDetails {
                    text: "Fix with Tilde Agent".to_owned(),
                    ...
                    if !self.autoupdate_unable_to_update_banner_dismissed =>
                {
                    let description =
                        if is_incoming_version_past_current(new_version.soft_cutoff.as_deref()) {
                            ...
                            "A new version is available but Tilde is unable to perform the update."
                                .to_owned()
                        };
                    ...
                    Some(WorkspaceBannerFields {
                        ...
                        secondary_button: None,
                        button: Some(WorkspaceBannerButtonDetails {
                            text: "Update Tilde manually".to_string(),
                    ...
                    if !self.autoupdate_unable_to_launch_new_version =>
                {
                    let description =
                        if is_incoming_version_past_current(new_version.soft_cutoff.as_deref()) {
                            ...
                            "Tilde was unable to launch the new installed version.".to_owned()
                        };
    ...
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            ...
            let toast_message = format!(
                "Have Tilde installed but redirecting to download page?\nEnable Local Network Access for {} in your browser.",
                ChannelState::server_root_url()
            );
            self.toast_stack.update(ctx, |toast_stack, ctx| {
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            CreatePersonalFolder | CreateTeamFolder => {}
            ...
            ViewObjectInWarpDrive(_) | OpenObjectSharingSettings { .. } | UndoTrash(_) => {}
            TerminateApp => {
            ...
            ToggleConversationListView | OpenConversationListView => {}
