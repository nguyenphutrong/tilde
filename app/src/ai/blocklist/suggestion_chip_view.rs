...
/// An [`ActionButton`] theme for suggested prompts.
...
/// Data required to render a suggested agent mode workflow.
#[derive(Clone)]
...
impl SuggestionChipView {
    ...
    fn handle_cloud_model_event(&mut self, event: &CloudModelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            | CloudModelEvent::ObjectDeleted {
                ...
                // If the workflow has been deleted, then we should reset it such that
    ...
    /// Resets the agent mode workflow to its initial state.
    ...
    /// Updates the UI state to reflect that a workflow has been added.
