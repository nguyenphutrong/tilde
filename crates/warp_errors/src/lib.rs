//! Local error diagnostics for Warp.
//!
...
//! that depends only on third-party crates, avoiding a dependency cycle with `warp_core`.
...
///
/// Actionable errors are logged at Error level; expected errors are logged at Warn level.
#[macro_export]
